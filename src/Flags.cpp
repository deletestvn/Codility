...
int solution(vector<int> &A) {
	...
	// This represents the maximum number of flags
	int maxFlag = sqrt(N) + 1;
	vector<int> peaks;
	...
	// If there is no peak, there is no flag
	if (peaks.size() == 0) return 0;
	int peaksNum = peaks.size();

	// If peaks is not empty, there exists at least one flag
	int count = 0;

	// For each possible gap, the condition for flags are:
	// 1. count cannot be more than gap 
	// 2. new flag and previous flag's distance is greater than gap

	// Loop through each possible max number of flag 
	for (int j = 1; j <= maxFlag; j++) {
		int previousFlag = -j;
		int candidate = 0;

		int k = 0;
		while (candidate < j && k < peaksNum) {
			if (peaks[k] - previousFlag >= j) {
				candidate += 1;
				previousFlag = peaks[k];
			}
			k++;
		}
