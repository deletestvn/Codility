...
vector<int> solution(vector<int> &A, vector<int> &B) {
	...
	// Generate all ways of climing for each rung number
	vector<int> fibonacci = { 1,2 };
	for (int i = 3; i <= L; i++) {
		int newFib = fibonacci[i - 2] + fibonacci[i - 3];
		fibonacci.push_back(newFib);
	}

	// Loop through each ladder and calculate the result
	unsigned int rungs, exponent, result;
	for (int j = 0; j < L; j++) {
		rungs = A[j];
		exponent = B[j];
		result = (fibonacci[rungs - 1]) % (unsigned int)pow(2, exponent);
		results[j] = result;
	}
