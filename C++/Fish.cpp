...
int solution(vector<int> &A, vector<int> &B) {
	int N = A.size();
	int alive = 0; // This keep track of fish swimming upstream which has eaten all possible downstream fish
	stack<int> standby; // This store the fish swimming downstream which has not been eaten
	
	int i = 0;
	while (i < N) {
		if (B[i] == 1) standby.push(i);
		else if (B[i] == 0 && standby.empty()) alive += 1;
		else if (B[i] == 0 && !standby.empty()) {
			// If the upstream fish is bigger than downstream fish
			if (A[i] > A[standby.top()]) {
				standby.pop();
				continue;
			}
			// If the upstream fish is smaller than downstream fish
		}
		i++;
	}
	return (alive + standby.size());
}