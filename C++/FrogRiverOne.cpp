...
int solution(int X, vector<int> &A) {
	...
	// Track the occurence of each element 1,2,...,X in A
	for (int i = 0; i < N; i++) {
		// The leaf falls into position currPosition at time i
		currPosition = A[i];
		// If this position has not been tracked
		if (track[currPosition-1] == false) {
			// Track this position
			track[currPosition-1] = true;
			// Show a position has been filled
			X--;
			// If all positions have been filled, we can return the current time
			if (X == 0) return i;
		}
	}
